...
[ -f $DOTFILES_ZSH_DIR/.zshenv.local ] && . $DOTFILES_ZSH_DIR/.zshenv.local
[ -f $ZDOTDIR/.zshenv.local ] && . $ZDOTDIR/.zshenv.local

# 環境ごとの設定 (.zshenv.darwin, .zshenv.linux)
UNAME_LOWERCASE=${$(uname):l}
[ -f $DOTFILES_ZSH_DIR/.zshenv.$UNAME_LOWERCASE ] && . $DOTFILES_ZSH_DIR/.zshenv.$UNAME_LOWERCASE

###############################################################################

# Pure
export PURE_PROMPT_SYMBOL="%B%F{3}❯%F{2}❯%F{6}❯%f%b"